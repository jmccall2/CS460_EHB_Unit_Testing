 ...
 * directly and explicitly knows about
 * It only guarantees one thing: it will call init().
 * If you need anything else you must set it up with the simulation.engine
 */
...
//    Kevin added the two below in order to have the ability for people to have the brake
//    hold without holding the button
        Engine.getMessagePump().registerMessage(new Message(SimGlobals.IS_DOWN));
        Engine.getMessagePump().registerMessage(new Message(SimGlobals.IS_UP));
        Engine.getMessagePump().registerMessage(new Message(SimGlobals.IS_PRESSED));
    ...
    private void _buildWorld()
    {
        ...
        String cloudPath = "simulator/resources/img/world/cloud.png";
       for(int i = 1; i <=6; i++)
       {
           BackgroundPanel bp = new BackgroundPanel("simulator/resources/img/world/background"+i+".jpeg",-1000 + (1000*(i-1)),-15,10,1000,700);
           bp.addToWorld();
    ...
    {
        SingleFrameEntity renderPanel = new SingleFrameEntity("simulator/resources/img/panelback.png",0,460,4,0,0,1020,240);
        renderPanel.setAsStaticActor(true);
        renderPanel.addToWorld();

        SingleFrameEntity blockingPanel = new SingleFrameEntity("simulator/resources/img/panelback.png",0,635,2,0,0,1020,70);
